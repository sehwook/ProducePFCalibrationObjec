...
ProducePFCalibrationObject::ProducePFCalibrationObject(const edm::ParameterSet& pSet) {
  ...
  outputfile_ = "calib_coeff_func.root"; 
  fout = new TFile(outputfile_.c_str(), "RECREATE"); 
}
...
void ProducePFCalibrationObject::beginRun(const edm::Run& run, const edm::EventSetup& eSetup) {
  ...
  if(read) {
    ...
    Int_t n_energy = 1000;
    Float_t x_energy[n_func][n_energy];
    Float_t y_coeff[n_func][n_energy];
    ...
    for(vector<string>::const_iterator name = fToRead.begin(); name != fToRead.end(); ++name) 
    {
      ...
      for (int energy=0; energy < n_energy;energy++)
      {
         ...
         x_energy[calib_func][energy] = energy;
         ...
         if(pfCalibrations->isInPayload(fType, point)) 
         {
           ...
           y_coeff[calib_func][energy] = value;
           cout << "   Energy:  " << x_energy[calib_func][energy] << " coeff.: " << y_coeff[calib_func][energy] << endl;
         } else {
            y_coeff[calib_func][energy] = 0;
            cout <<  "outside limits!" << endl;
         }
      }

      
      gcalib_func[calib_func] = new TGraph(n_energy, x_energy[calib_func], y_coeff[calib_func]);
      gcalib_func[calib_func]->Clear();
      gcalib_func[calib_func]->SetName((*name).c_str());
      gcalib_func[calib_func]->SetNameTitle((*name).c_str(), (*name).c_str());
      gcalib_func[calib_func]->Write();
...
void ProducePFCalibrationObject::Book()
{
}
